...
- The Docker runtime (`hypr-devns-runc`) is registered as the **default OCI runtime**, so all containers are affected. Revert with `hypr-devns-disable`.
...
After installing, run the one-time setup to integrate with your desktop:
...
To undo everything: `hypr-devns-disable`
